package in.lalit.config.product;
...
@EnableJpaRepositories(
		...
		basePackages = "in.lalit.repo.product"
		)
public class Db2Config {
	
	//DataSource
	@Primary // to choose which to give preference to select for database
	@Bean
	...
	public LocalContainerEntityManagerFactoryBean db2EntityManagerFactory(EntityManagerFactoryBuilder emfb) {
		...
		return emfb.
				...
				packages("in.lalit.config.model.product").
				properties(properties).
				persistenceUnit("apples").
				build();
	}
