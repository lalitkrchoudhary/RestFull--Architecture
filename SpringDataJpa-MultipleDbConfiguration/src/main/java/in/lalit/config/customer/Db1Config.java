package in.lalit.config.customer;
...
@EnableJpaRepositories(
		...
		basePackages = "in.lalit.repo.customer"
		)
public class Db1Config {
	...
	public LocalContainerEntityManagerFactoryBean db1EntityManagerFactory(EntityManagerFactoryBuilder emfb) {
		...
		return emfb.dataSource(db1DataSource()).packages("in.lalit.config.model.customer").properties(properties).persistenceUnit("apple").build();
	}
